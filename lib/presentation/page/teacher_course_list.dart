...
class _TeacherCourseListState extends State<TeacherCourseList> with TickerProviderStateMixin{
  ...
  final CourseListProv clProv = ProvManager.courseListProv;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // if(urgingStatus(clProv.status)){
      //   clProv.updateCourseList();
      // }
      clProv.updateCourseList();
    });
  }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Expanded(
              child: Selector<CourseListProv, DataStatus>(
                ...
                builder: (_,status,__) {
                  switch (status) {
                    ...
                    case DataStatus.failure:
                      return ErrorPage(
                        onRetry: clProv.updateCourseList,
                      );
                    case DataStatus.success:
                      return Selector<CourseListProv, int>(
                        selector: (context, prov) => prov.courses.length,
                        shouldRebuild: (prev, next) => true,
                        builder: (_,__,___){
                          return ListView.builder(
                              itemCount: clProv.courses.length,
                              itemBuilder: (context,index){
                                CourseBrief cb = clProv.courses[index];
                                return Padding(
                                            ...
                                            children: [
                                              ...
                                              Wrap(
                                                ...
                                                children: [
                                                  OutlinedButton(
                                                    onPressed: (){
                                                      ProvManager.courseDetailProv.setCourseInstId(cb.courseInstId);
                                                      // ProvManager.courseDetailProv.fetchCourseDetail();
                                                  ...
                                                  OutlinedButton(
                                                    onPressed: (){
                                                      ProvManager.studentScoreProv.setCourseInstId(cb.courseInstId);
                                                      Navigator.pushNamed(context, '/sscore');
                                                    },
                                                    style: OutlinedButton.styleFrom(
                                                      side: BorderSide(
                                                        color: index.isOdd ? const Color(0xff4e34d4): context.theme.colorScheme.primary,
